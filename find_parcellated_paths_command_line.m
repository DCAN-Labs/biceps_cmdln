function handles = find_parcellated_paths(handles, validate_frame_counts, varargin)
fs=filesep;
...
if validate_frame_counts == -1
    ...
    answer = questdlg(prompt, ...
        ...
        'Yes','No','Yes');
elseif validate_frame_counts > 0
    answer = 'No';
else
    answer = 'Yes';
end
...
rest_naming_convention = '*rest*bold*';
if nargin >= 3
    rest_naming_convention = varargin{1};
end

%Load optional search term for atlas descriptor. (i.e. 'parc-'). Add
%dash at the end if it isnt given by user
if nargin == 4
    additional_atlas_search_term = varargin{2};
    if strcmp(additional_atlas_search_term(end), '-') == 0
        rest_naming_convention = [additional_atlas_search_term '-'];
    end
end
...
if strcmp(rest_naming_convention(0), '*') == 0
    rest_naming_convention = ['*' rest_naming_convention];
end

search_term = [rest_naming_convention '.ptseries.nii'];
...
for k=1:n
    ...
    dummy=dir([path_to_nii  fs search_term]);
    ...
    for x = 1:n_nii
        ...
        if size(pt) < 2
            ...
            if (size(pt) < 2) && (nargin == 4)
                pt = split(nii_files{x}, additional_atlas_search_term);
