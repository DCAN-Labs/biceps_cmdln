...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @biceps_subf_inserted_OpeningFcn, ...
    'gui_OutputFcn',  @biceps_subf_inserted_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function biceps_subf_inserted_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = biceps_subf_inserted_OutputFcn(hObject, eventdata, handles)
...
set(handles.path_group_file_text,'string', handles.groupFile);
...
set(handles.text_survivors,'string',make_text_survivors(handles)); %looks like this is only needed for GUI
...
temp=str2double(get(hObject,'String'));
handles = subf_edit_min_time_minutes_Callback(handles, temp);
...
temp=str2double(get(hObject,'String'));
handles = subf_edit_skip_Callback(handles, temp);
...
handles = subf_checkbox_detect_outliers_Callback(handles, 1);
set(handles.text_survivors,'string',make_text_survivors(handles));
handles.output = hObject;
guidata(hObject, handles);
...
for i=1:n
    ...
    disp(handles.mc.surv_parcels{i});
end
...
% handles    structure with handles and user data (see GUIDATA)
handles.save_bids = 0;
handles = subf_pushbutton_make_environments_Callback(handles, 1, 0);
handles.output = hObject;
guidata(hObject, handles);
